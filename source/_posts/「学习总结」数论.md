...
$\begin{equation}\label{Lucas}\dbinom{n}{m} \equiv \prod_{i \ge 0}\limits{\dbinom{a_i}{b_i}} ( mod\ P)\end{equation}$
同时 $(\ref{Lucas})$ 还有一种形式化的写法：
...
关于函数 $g_p(n)$ 的计算：
$g_p(n) = \prod_{i\ge 1}\limits{\lfloor\frac{n}{p^i}\rfloor}= \frac{n - f_p(n)}{p - 1}$，其中 $f_p(n)$ 为数字 $n$ 在 $p$ 进制下的数位和。

注意逆元不是质数，别<del>傻不拉几的</del>冲一个费马小定理。
