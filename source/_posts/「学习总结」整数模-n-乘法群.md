...
类似地定义原根不存在的 $\mathbb{Z}_m^*$，设 $m=\prod_{i=1}^{s}\limits{P_i^{e_i}}$ 。他们的 “离散对数” 形成的群可以表示为 $G_{P_1^{e_1}}\times G_{P_2^{e_2}} \times G_{P_3^{e_3}}\times\cdots\times G_{P_s^{e_s}}$ ，其中 $\times​$ 为定义在群上的直积。
...
定义 $n=(a, c),m=(b, d)$。任何一个数字 $n,m$ 都可以用形如 $(x, y)$ 的数对表示。同时，$\forall x,y \in [0, P-2]\ \ (x, y)$ 的数对都唯一的对应一个在 $[1, P(P-1)]$ 的数字。
